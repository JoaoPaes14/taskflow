...
@Component({
  ...
  styles: [`
    :host { display: flex; height: 100vh; background: var(--bg); color: var(--text); }
    ...
    .stat-card {
      ...
      strong { display: block; font-size: 1.4rem; margin-bottom: 0.2rem; }
    }
    .filters { display: flex; gap: 0.5rem; margin-bottom: 1rem; flex-wrap: wrap; }
    .filter-btn {
      padding: 0.4rem 0.85rem; border-radius: 20px; border: 1px solid var(--border);
    ...
    .task-row {
      ...
      &:hover { box-shadow: var(--shadow-md); border-color: rgba(99, 102, 241, 0.2); }
    }
    .status-badge {
      width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0;
      &.todo { background: #f59e0b; }
      &.in-progress { background: #8b5cf6; }
      &.done { background: #10b981; }
    }
    .task-info { flex: 1; min-width: 0; }
    .task-title { font-size: 0.88rem; font-weight: 600; }
    .task-project { font-size: 0.72rem; color: var(--text-muted); margin-top: 0.15rem; }
    .task-due {
      ...
      &.future { background: var(--bg); color: var(--text-muted); }
    }
    .priority-badge {
