...
@Component({
    ...
    <div class="calendar-section">
      ...
      <div class="cal-grid">
        ...
        @for (cell of calendarCells(); track cell.date) {
          <div class="cal-cell" [class.other-month]="cell.isOtherMonth" [class.today]="cell.isToday">
            <span class="cal-date">{{ cell.day }}</span>
            @if (cell.tasks.length > 0) {
              <div class="cal-tasks">
                @for (t of cell.tasks.slice(0, 3); track t.id) {
                  <div class="cal-task" [class]="'cal-task-' + t.status.toLowerCase()" [title]="t.title">
                    {{ t.title }}
                  </div>
                }
  ...
  styles: [`
    ...
    .cal-nav {
      ...
      &:hover { color: var(--text); border-color: var(--primary); }
    }
    .cal-grid {
    ...
    .cal-day-header {
      background: var(--bg); padding: 0.5rem; text-align: center;
      font-size: 0.72rem; font-weight: 700; color: var(--text-muted);
    }
    .cal-cell {
      background: var(--surface); min-height: 80px; padding: 0.35rem;
      &.other-month { background: var(--bg); opacity: 0.5; }
      &.today { background: var(--primary-soft); }
    }
    .cal-date {
    ...
    .cal-task {
      ...
      &.cal-task-todo { background: var(--warning-soft); color: #d97706; }
      &.cal-task-in_progress { background: var(--primary-soft); color: var(--primary); }
      &.cal-task-done { background: var(--success-soft); color: #059669; }
    }
    .cal-more { font-size: 0.6rem; color: var(--text-muted); padding: 0 4px; }
  `]
})
export class CalendarComponent implements OnChanges {
  ...
  private buildCalendar(): { date: string; day: number; isOtherMonth: boolean; isToday: boolean; tasks: ProjectTask[] }[] {
    ...
    const cells: { date: string; day: number; isOtherMonth: boolean; isToday: boolean; tasks: ProjectTask[] }[] = [];
